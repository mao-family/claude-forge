...
CONFIG_FILE="$(dirname "$0")/../config/workspace.json"
...
select_repos() {
    ...
    # Get repo list
    repos=$(gh repo list "$org" --limit 100 --json name,description --jq '.[] | "\(.name)|\(.description)"' 2>/dev/null)
    ...
    for repo in $selected; do
        repo_path="$WORKSPACE_DIR/$org/$repo"
        if [ -d "$repo_path" ]; then
...
gh auth status 2>&1 | grep -E "(Logged in|Active)" | head -10
echo ""
